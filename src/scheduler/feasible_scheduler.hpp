...
struct scheduler_traits {
  ...
  //////////////////////////////////////////////////////////////////////////////
  // DAG editor traits //
  typedef int dag_editor_t;
  // operation/node factory //
  static operation_t new_operation(dag_editor_t&, const std::string&); 
  static operation_t new_operation(dag_editor_t&, const std::string&,
      operation_t orig_op, resource_t); 
  static void add_edge(dag_editor_t&,
        const operation_t& src, const operation_t& sink);
  static void remove_edge(dag_editor_t&,
        const operation_t& src, const operation_t& sink);
  static const dag_t& get_dag(const dag_editor_t&);
  static void apply_edits(const dag_editor_t&, dag_t&);
  //////////////////////////////////////////////////////////////////////////////
...
class Feasible_Schedule_Generator {
  ...
  {
    if (!schedulable_op_)
      throw "Feasible_Schedule_Generator: Null ptr dereference";
...
class Feasible_Memory_Schedule_Generator {
  public:
    ...
    struct scheduled_op_info_t {
      ...
      void reset_as_original_op() {
        op_type_ = op_type_e::ORIGINAL_OP;
      }
      ...
      void print() const {
        printf("op=%s time=%lu type=%s resource=",
              traits::operation_name(op_), time_, op_type_name());
        if (has_active_resource()) {
          std::cout << "[" << begin_resource() << " " << end_resource() << "]" 
            << std::endl;
        } else {
